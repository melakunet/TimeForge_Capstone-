...
// Resolve logo for PDF — Dompdf needs base64-embedded images (cannot load URLs)
$tf_logo_path      = __DIR__ . '/../icons/logo.png';
$company_logo_path = (!empty($invoice['creator_logo']))
    ? __DIR__ . '/../' . $invoice['creator_logo']
    : null;

function pdfLogoBase64(string $path): string {
...
// Use company logo if uploaded, else fall back to TimeForge logo
$pdf_logo_b64    = ($company_logo_path && file_exists($company_logo_path))
    ? pdfLogoBase64($company_logo_path)
    : pdfLogoBase64($tf_logo_path);
$pdf_tf_logo_b64 = pdfLogoBase64($tf_logo_path);  // always the TimeForge logo for footer
...
<div class="page">
  ...
  <table class="hdr-table" cellspacing="0" cellpadding="0">
    <tr>
      <td style="width:60%;">
        <?php if ($pdf_logo_b64): ?>
          <img src="<?php echo $pdf_logo_b64; ?>" alt="logo" style="max-height:48px; max-width:120px; margin-bottom:6px; display:block;">
        <?php endif; ?>
        <div class="brand-name"><?php echo htmlspecialchars($invoice['creator_company'] ?: ($invoice['creator_name'] ?? '')); ?></div>
  ...
  <div class="body-pad">
    ...
    <table class="addr-table" cellspacing="0" cellpadding="0">
      <tr>
        <td>
          ...
          <?php endif; ?>
          <?php if (!empty($invoice['creator_tagline'])): ?>
            <?php echo htmlspecialchars($invoice['creator_tagline']); ?><br>
          <?php endif; ?>
          <?php if (!empty($invoice['creator_email'])): ?>
            <?php echo htmlspecialchars($invoice['creator_email']); ?><br>
          <?php endif; ?>
        </td>
        <td>
    ...
    <div class="foot-note" style="display:table; width:100%;">
      <div style="display:table-cell; text-align:left; vertical-align:middle;">
        Thank you for your business &mdash; <?php echo htmlspecialchars($invoice['client_name']); ?><?php if (!empty($invoice['client_company'])): ?> / <?php echo htmlspecialchars($invoice['client_company']); ?><?php endif; ?>.<br>
        Please remit payment by <?php echo date('F j, Y', strtotime($invoice['due_date'])); ?>.
      </div>
      <div style="display:table-cell; text-align:right; vertical-align:middle; white-space:nowrap; padding-left:12px;">
        <?php if ($pdf_tf_logo_b64): ?>
          <img src="<?php echo $pdf_tf_logo_b64; ?>" alt="TimeForge" style="height:12px; width:12px; vertical-align:middle; opacity:0.5; margin-right:3px;">
        <?php endif; ?>
        <span style="font-size:8px; color:#9ca3af;">Powered by <strong>TimeForge</strong></span>
      </div>
    </div>
